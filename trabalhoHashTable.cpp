...
//mandei dois,mas considere esse por favor , na verdade estao iguais a unica diferenca e que na funcao para remover e buscar um lote e pedido o numero do lote sendo que tem que ser colocado o numero de hash do lote
#include <iostream>
#include<string>
using namespace std;
...
class ListaProdutor
{
	...
	public:
		...
		void remover()
		{
			if (!getvazia())
			{
				...
				else // > 2 elementos
				{
					...
					delete ant_ant->prox; // libera mem�ria
		...
		{
			if (getvazia()) // se for vazia, ent� retorna 0
				return 0;
...
class TabelaHashLote
{
	...
	public:
		...
		void insereLote (string numeroDoLote, string dataProducao, string nomeDoProduto ,int CNPJdoProdutor) {
			...
			if (buscarLote(hash) == "NAO ENCONTRADO" )
			{
	...
	do
	{
		...
		if (opcao == 2)
		{
			do
			{
				...
				{
					cin.ignore();
					cout << "Qual e o numero de hash lote que deseja buscar " << endl;
					cin >> numeroHash;
				...
				{
					cin.ignore();
					cout << "Qual e o numero de hash lote que deseja remover " << endl;
					cin >> numeroHash ;
